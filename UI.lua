...
-- Autor: Lleguito | Version: 3.1 | WotLK 3.3.5
-- ============================================================
...
local function RefreshBotList()
    ...
    for _, botName in ipairs(names) do
        ...
        -- Buscar e incrustar icono de talento en la lista lateral
        local entry = NBI.botEntryByName and NBI.botEntryByName[botName]
        local iconStr = ""
        if entry and NBI.SPEC_INFO then
            local botSpec = NBI.botRoles[entry]
            local info = NBI.SPEC_INFO[botSpec]
            if info and info.icon then
                iconStr = "|T" .. info.icon .. ":13:13:0:0|t "
            end
        end
